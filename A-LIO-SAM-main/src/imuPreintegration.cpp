        ...
        if (!doneFirstOpt) // use the ini time estimate grav direction
        {
            ...
            if (ROS_TIME(&imuQueGrav.back()) - ROS_TIME(&imuQueGrav.front()) > gravIniTime)
            {
                ...
                if (integral > 0.0)
                {
                    ...
                    Eigen::Quaterniond qInertial2IMU = Eigen::Quaterniond::FromTwoVectors(integratedGrav, -inertialGrav);
                    Eigen::Quaterniond qLiDAR2IMU(extRot);
                    Eigen::Quaterniond qInertial2LiDAR = qInertial2IMU * qLiDAR2IMU.inverse();
                    Eigen::Vector3d transIniLiDAR = qInertial2LiDAR * Eigen::Quaterniond(offsetRot).inverse() * offsetTrans;
